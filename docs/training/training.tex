...
\begin{tabular}{| l | p{12.6cm} |}
	...
	onderwerp & Een pagina met een andere lay-out (andere regio's) dan een pagina. Dit inhoudstype wordt gebruikt om een overzicht te geven van een bepaald onderwerp. \\ \hline
	...
	peiling & Een vraag die aan bezoekers van de website wordt gesteld (ook wel poll
		genoemd). \\ \hline
	webformulier & Een formulier dat bezoekers van de website kunnen invullen (formulieren in digitaal loket c.q. e-loket worden niet in Drupal geconfigureerd). \\ \hline
\end{tabular}
...
\item U wordt gevraagd aan te geven of het gaat om een "publiek lokaal bestand aangeboden door de webserver"\ of een "\,afgeschermd lokaal bestand aangeboden via Drupal". In het eerste geval (publiek bestand) wordt het bestand voor iedereen benaderbaar. Een afgeschermd bestand is alleen benaderbaar voor ingelogde gebruikers (met rechten op het domein). Kies voor de publieke internetsite altijd voor 'publiek bestand'. Klik vervolgens op \emph{Volgende}.
...
De redacteur is nu klaar met het toevoegen van een nieuws-item, maar deze staat nog niet op de website gepubliceerd (let er op dat deze wel zichtbaar is voor (eind)redacteuren die zijn ingelogd). De rest van het scenario wordt uitgevoerd door de eindredacteur.
