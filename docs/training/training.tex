...
Er zijn nog diverse andere inhoudstypen. Een deel van de inhoud wordt geïmporteerd. Producten, bekendmakingen en regelingen zijn hier een voorbeeld van. De volledige lijst met inhoudstypen is te vinden in de Dimpact-handleiding (\S2.8 'inhoudstypen').
...
\item U hebt nu een pagina aangemaakt waar de editorial kan worden toegevoegd. Ga met de muisaanwijzer naar het tandwiel binnen het blok "Regio 4/12 (3/5/4) Node", rechts op de pagina. Klik op het tandwiel en kies \emph{Blok toevoegen}.
...
Welke afbeeldingen er in een blok worden getoond is gedefinieerd in een \emph{nodequeue}. Een \emph{nodequeue} is een geordende lijst met inhoud. Deze inhoud hoeft niet het type \emph{slide} te hebben; het is bijvoorbeeld ook mogelijk een \emph{nodequeue} aan te maken die naar pagina's verwijst, waarna deze als een lijst met links in een blok kan worden toegevoegd.

Het systeem kent ook een \emph{list}, wat een lijst is die dynamisch wordt opgesteld aan de hand van bepaalde criteria; het is bijvoorbeeld mogelijk een \emph{list} aan te maken van alle inhoud van het type \emph{slide}, waar een bepaalde tag aan is toegevoegd.
...
Als beheerder kunt u nazoeken welke nodequeue aan een carrousel is gekoppeld. Klik hiervoor op het tandwieltje rechtsboven in de carrousel en vervolgens op \emph{Overzicht bewerken}. U vindt nu rechts onder \emph{Uitgebreid} en \emph{Relaties} de tekst "Nodequeue: Wachtrij".

\begin{center}
	\includegraphics[scale=0.4]{img/carrousel_admin.png}
\end{center}

Wanneer u op "Nodequeue: Wachtrij"\ klikt verschijnt een venster met o.a. een lijst van \emph{queues}. Hier staat aangevinkt welke nodequeues in de carrousel worden weergegeven.
