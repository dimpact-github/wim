      ...
      if (domain_conf_variable_get($_domain['domain_id'], 'css_killswitch')) {
        foreach (dimpact_custom_css_files() as $key => $title) {
          ...
          if ($key == 'custom_css_screen') {
            print '<link rel="stylesheet" href="' . $base_url . '/' . conf_path() . '/files/custom_css/' . $file->filename . '" media="screen" />';
          }
          if ($key == 'custom_css_print') {
            print '<link rel="stylesheet" href="' . $base_url . '/' . conf_path() . '/files/custom_css/' . $file->filename . '" media="print" />';
          }
        }
      }
    ?>
    ...
    <?php if (domain_conf_variable_get($_domain['domain_id'], 'css_killswitch')) {
        foreach (dimpact_custom_css_files() as $key => $title) {
          $file = file_load(domain_conf_variable_get($_domain['domain_id'], $key));
            ...
            print '<!--[if lte IE 9]><link rel="stylesheet" href="' . $base_url . '/' . conf_path() . '/files/custom_css/' . $file->filename . '" media="all" /><![endif]-->';
          }
          if ($key == 'custom_css_ltie9') {
            print '<!--[if lt IE 9]><link rel="stylesheet" href="' . $base_url . '/' . conf_path() . '/files/custom_css/' . $file->filename . '" media="all" /><![endif]-->';
          }
        }
      } 
    ?>
